...
public: // methods (some exceptions)
    ...
    void setDenominator(int d)
    {
        ...
        catch (int invalidDenominator)
        {
            cout << "Problem encountered; invalid denominator. Setting to 1." << endl;
            denominator = 1;
        }
    }
...
{ // pass-by-value; none of the "extra stuff"
    // a/b + c/d = (ad + bc)/bd
    Fraction result;
    // step-by-step
...
{
    // ostream& - returning reference. Why?
    /*
    ostream = dealing with output, this particular thing could be in the
    middle of a series of outputs
    ...
    Fraction f;
    cout<<x<<" "<<y<<" "<<f<<endl;
    // f is a part of this stream - why ostream& as return type

    ostream - class of output. ofstream << , cout <<
    */
    //    print out the numerator/denominator
...
{
    // assignment operator
    // copy the values of f2 into f1
    f1.setNumerator(f2.getNumerator());
    f1.setDenominator(f2.getDenominator());
    return f1;
}
...
{
    // input the numerator/denominator
    int n, d;
    char slash;
    is >> n >> slash >> d;
    f.setNumerator(n);
    f.setDenominator(d);
    f.publicReduce(); // make sure that fractio is in its reduced form
    return is;
}
...
int main()
{
    ...
    Fraction f1, f2;
    cout << "Enter the first fraction (n/d): ";
    cin >> f1;
    cout << "Enter the second fraction (n/d): ";
    cin >> f2;
    cout << "The first fraction is: " << f1 << endl;
    cout << "The second fraction is: " << f2 << endl;
